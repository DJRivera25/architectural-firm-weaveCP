...
import EmployeeKanbanBoard from "../../KanbanBoard";
import { getProjects } from "@/utils/api";
import type { Project } from "@/types";

const EmployeeBoardPage: React.FC = () => {
  const params = useParams();
  const router = useRouter();
  const projectId = params?.projectId as string;
  const [project, setProject] = useState<Project | null>(null);
  const [loading, setLoading] = useState(true);
  const [taskCounts, setTaskCounts] = useState({ total: 0, assigned: 0 });
  ...
  return (
    <div className="min-h-screen flex flex-col " style={backgroundStyle}>
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6 rounded-t-xl">
        <EmployeeKanbanBoard projectId={projectId} onTaskCountsChange={setTaskCounts} />
      </div>
      <button
