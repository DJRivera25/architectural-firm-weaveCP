...
import { ArrowLeftIcon, ChartBarIcon, CalendarIcon, PencilIcon, TrashIcon } from "@heroicons/react/24/outline";
...
import { getProjects, getTasks, getTimeLogs, updateTimeLog, deleteTimeLog } from "@/utils/api";
...
interface ClockifyPageProps {
  params: Promise<{
    projectId: string;
  }>;
}

const EmployeeClockifyPage: React.FC<ClockifyPageProps> = ({ params }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [projectsResponse, tasksResponse, timeLogsResponse] = await Promise.all([
          ...
          getTimeLogs(`?projectId=${projectId}&userId=${session?.user?.id}`),
        ]);

        // Also fetch all time logs for accurate task total calculation
        const allTimeLogsResponse = await getTimeLogs(`?projectId=${projectId}`);
        ...
        // Sort user's time logs by latest first (for display)
        const sortedUserTimeLogs = (timeLogsResponse.data || []).sort(
          (a: TimeLogData, b: TimeLogData) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()
        );

        // Sort all time logs by latest first (for task total calculation)
        const sortedAllTimeLogs = (allTimeLogsResponse.data || []).sort(
          (a: TimeLogData, b: TimeLogData) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()
        );

        // Use user's time logs for display
        setTimeLogs(sortedUserTimeLogs);

        // Calculate task total times based on ALL time logs (not just user's)
        const tasksWithCalculatedTotals = userTasks.map((task: Task) => {
          const taskTimeLogs = sortedAllTimeLogs.filter((log: TimeLogData) => log.taskId === task._id);
          const calculatedTotalTime = taskTimeLogs.reduce((sum, log) => sum + (log.duration || 0), 0);
          return { ...task, totalTime: calculatedTotalTime };
        });

        setAssignedTasks(tasksWithCalculatedTotals);
      } catch (err) {
  ...
  const handleTimeLogCreated = (newTimeLog: TimeLogData) => {
    console.log("handleTimeLogCreated called:", newTimeLog);
    setTimeLogs((prev) => [newTimeLog, ...prev]);

    // Update task's totalTime if this time log is for a task
    if (newTimeLog.taskId) {
      setAssignedTasks((prevTasks) =>
        prevTasks.map((task) => {
          if (task._id === newTimeLog.taskId) {
            // Add the new time log duration to the existing total
            const newTotalTime = (task.totalTime || 0) + (newTimeLog.duration || 0);
  ...
  const handleTimeLogUpdated = (updatedTimeLog: TimeLogData) => {
    console.log("handleTimeLogUpdated called:", updatedTimeLog);
      ...
      setAssignedTasks((prevTasks) =>
        prevTasks.map((task) => {
          if (task._id === updatedTimeLog.taskId) {
            ...
            // We need to fetch all time logs for this task to get accurate total
            const recalculateTaskTotal = async () => {
              try {
                const allTimeLogsResponse = await getTimeLogs(
                  `?projectId=${projectId}&taskId=${updatedTimeLog.taskId}`
                );
                const allTaskTimeLogs = allTimeLogsResponse.data || [];
                const newTotalTime = allTaskTimeLogs.reduce((sum, log) => sum + (log.duration || 0), 0);

                setAssignedTasks((currentTasks) =>
                  currentTasks.map((currentTask) => {
                    if (currentTask._id === updatedTimeLog.taskId) {
                      return { ...currentTask, totalTime: newTotalTime };
                    }
                    return currentTask;
                  })
                );
              } catch (err) {
                console.error("Error recalculating task total:", err);
              }
            };

            recalculateTaskTotal();
            return task; // Return unchanged for now, will be updated by the async function
          }
          return task;
        })
      );
  ...
  const handleBack = () => router.push(`/employee-dashboard/productivity/board/${projectId}`);
  ...
  const canEditTimeLog = (timeLog: TimeLogData) => {
    if (!session?.user) {
      console.log("canEditTimeLog: No session user");
      return false;
    }
    ...
    if (typeof timeLog.userId === "string") {
      ...
      console.log("canEditTimeLog: Invalid userId format:", timeLog.userId);
      return false;
    }

    const canEdit = session.user.id === timeLogUserId;
    console.log("canEditTimeLog check:", {
      sessionUserId: session.user.id,
      timeLogUserId: timeLogUserId,
      originalTimeLogUserId: timeLog.userId,
      canEdit,
  ...
  const handleSaveTimeLog = (updatedTimeLog: TimeLogData) => {
    ...
    if (updatedTimeLog.taskId) {
      ...
      const recalculateTaskTotal = async () => {
        try {
          const allTimeLogsResponse = await getTimeLogs(`?projectId=${projectId}&taskId=${updatedTimeLog.taskId}`);
          const allTaskTimeLogs = allTimeLogsResponse.data || [];
          const newTotalTime = allTaskTimeLogs.reduce((sum, log) => sum + (log.duration || 0), 0);
  ...
  const handleDeleteTimeLog = async (timeLogId: string) => {
    ...
    try {
      ...
      // Find the time log to get its taskId before deleting
      ...
      if (taskId) {
        // Recalculate total time for this task based on all remaining time logs
        const recalculateTaskTotal = async () => {
          try {
            const allTimeLogsResponse = await getTimeLogs(`?projectId=${projectId}&taskId=${taskId}`);
            const allTaskTimeLogs = allTimeLogsResponse.data || [];
            const newTotalTime = allTaskTimeLogs.reduce((sum, log) => sum + (log.duration || 0), 0);
  ...
  // Filter time logs based on task filter and sort by latest first
  ...
  // Debug: Log current time logs state
  console.log("Current time logs state:", {
    timeLogsCount: timeLogs.length,
    filteredTimeLogsCount: filteredTimeLogs.length,
    sessionUserId: session?.user?.id,
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => router.push(`/employee-dashboard/productivity/clockify/${projectId}/summary`)}
                ...
                <button
                  onClick={() => router.push(`/employee-dashboard/productivity/clockify/${projectId}/calendar`)}
                  className="flex items-center gap-2 px-4 py-2 text-gray-600 hover:text-gray-900 transition-colors"
                >
                      ...
                      <div key={task._id} className="border rounded-lg p-4 bg-gray-50">
                        ...
                        <div className="text-sm text-gray-600 mb-3">
                          {task.totalTime && task.totalTime > 0 && (
                            <span className="text-blue-600 font-medium">Total: {formatTimeHuman(task.totalTime)}</span>
                          )}
                        </div>
                ...
                {filteredTimeLogs.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {filteredTimeLogs.map((timeLog) => {
                      const canEdit = canEditTimeLog(timeLog);
                      console.log("Rendering time log:", {
                        timeLogId: timeLog._id,
                        description: timeLog.description,
                        userId: timeLog.userId,
                        sessionUserId: session?.user?.id,
                        canEdit,
                        willShowButtons: canEdit,
                      });
...
export default EmployeeClockifyPage;
