...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let finalData = { ...form, attachments: uploadedAttachments } as Task;

      // For employees, filter out restricted fields before sending to API
      if (isEmployee) {
        // Only allow these specific fields for employees
        const allowedFields = ["status", "attachments", "checklist"];
        finalData = Object.fromEntries(
          Object.entries(finalData).filter(([key]) => allowedFields.includes(key))
        ) as Task;

        console.log("TaskModal: Filtered data for employee:", finalData);
      } else {
            ...
            <div className={`flex-1 flex flex-col md:flex-row sm:max-h-[70vh] max-h-[60vh] `}>
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-2">
                  <input
                    ...
                    className={`text-3xl font-bold text-gray-900 bg-transparent border-none outline-none focus:ring-0 w-full ${
                      isEmployee ? "cursor-not-allowed opacity-75" : ""
                    }`}
                  />
                </div>
                ...
                <div className="relative">
                  ...
                  {/* Members Dropdown */}
                  {openDropdown === "members" && !isEmployee && (
                    <MembersDropdown
                      users={users}
                ...
                {(form.dueDate || showDueDateSection) && (
                  <div className="mb-4">
                    ...
                    <div className="relative">
                      <div
                        className={`border rounded px-3 py-2 transition-colors flex items-center justify-between ${
                          isEmployee ? "cursor-not-allowed opacity-75 bg-gray-100" : "cursor-pointer hover:bg-gray-50"
                        }`}
                        onClick={isEmployee ? undefined : () => setShowDatePicker(!showDatePicker)}
                      >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`border rounded px-3 py-2 w-full min-h-[80px] focus:ring-2 focus:ring-blue-200 ${
                      isEmployee ? "cursor-not-allowed opacity-75 bg-gray-100" : ""
                    }`}
                    rows={4}
                  />
                </div>
